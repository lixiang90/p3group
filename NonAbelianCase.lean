...
private lemma semidirectP2P_of_exponent_p2 (hexp : Monoid.exponent G = p ^ 2)
    ...
    -- find a new pair (x', y')=(x, y^r)
    -- such that y' * x' * y'⁻¹ = x' ^ (1 + p)
    -- Then, prove y' * x' * y'⁻¹ = x' ^ (1 + p) for this new pair.
    -- where r is inverse of m_nat in Z(p)
